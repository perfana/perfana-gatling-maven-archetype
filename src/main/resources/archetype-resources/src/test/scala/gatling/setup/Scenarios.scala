...
import ${package}.gatling.configuration.Configuration
...
object Scenarios {
  ...
  val acceptanceTestScenario = scenario("Acceptance test").feed(SomeDataFeeder.someData)
    .exec(session => session.set("testRunId", Configuration.testRunId))  // inject testRunId into session to be used in Dynatrace headers
    .exitBlockOnFail(
      randomSwitch(
